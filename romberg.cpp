...
void imprimir(double A[MAX][MAX]){
    for(int i=0;i<MAX;i++){
        for(int j=0;j<MAX;j++){
            cout<<"("<<i<<";"<<j<<"): "<<A[i][j]<<" ";
        }
...
double romberg(double a,double b,double (*f)(double),int n){
    double R[MAX][MAX];
    n=1;double ea;
    ...
    R[0][0]=trapecio(a,b,f,n);
    do{
        i++;
        n=(int)pow(2,i);
        R[i+1][1]=trapecio(a,b,f,n);
        for(int k=2;k<=i+1;k++){
            j=2+i-k;
            R[j][k]=(pow(4,k-1)*R[j+1][k-1]-R[j][k-1])/(pow(4,k-1)-1);

        }
        ea=abs((R[1][i+1]-R[1][i])/R[1][i+1])*100;
    }while(i<MAX);
    imprimir(R);
    return R[1][i+1];
}
...
double romberg(double a,double b,double (*f)(double),int n){
    ...
    R[0][0]=trapecio(a,b,f,n);
    for(int i=1;i<MAX;i++){
        for(int k=1;k<=pow(2,MAX-1);k++){
            sum+=f(a+(2*k-1)*h);
        }
        R[i][0]=(1/2)*R[i-1][0]+h*sum;
        for(int j=1;j<MAX;j++){
...
void viewTable(int n,double &a,double &b)
{
    ...
    cout<<endl;
    cout<<"\n";
    cout<<" n"<<'\t'<<"trapecio"<<'\t'<<"Simpson 1/3"<<'\t'<<"Simpson 3/8"<<'\t'<<"Romberg"<<endl;
    cout<<"\n";
    cout<<"-------------------------------------------------------------"<<endl;
    
    for(int i=6;i<193;i=i*2)
    {
        cout<<" "<<i<<"\t"<<trapecio(a,b,f,i)<<"     \t"<<Simpson13(a,b,f,i)<<"    \t"<<Simpson38(a,b,f,i)<<"    \t"<<romberg(a,b,f,i)<<endl;
        //cout<<i<<" ";
    }
}
int main()
