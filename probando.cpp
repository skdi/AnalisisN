...
/*Newton lab3*/
double fx(double x){
    return (pow(x,3)+3*pow(x,2)*cos(x)-1);
}
double dfx(double x){
    float h=0.0001;
...
void main_newton(){
    ...
    double ea;
    double x0=-2,x1=2,x2;
    double error=0.001;//10^-3
    ea=error+1;
    int n=0;
    while(ea>error){

        x2=(x1-((x0-x1)/(fx(x0)-fx(x1))*fx(x1)));
        ea=(fabs(x1-x2));
        x0=x1;
        x1=x2;
        cout<<n<<"| x1:"<<x1<<" x2:"<<x2<<" Error: "<<ea<<endl;
        n++;
    }
...
float normax(float vector1[],float vector2[],int c){
    float aux=0,aux2=0;
    int i;
    for(i=0;i<c;i++){
        aux+=pow(vector1[i],2);
        aux=sqrt(aux);
        aux2+=pow(vector2[i],2);
        aux2=sqrt(aux2);
    }
    return abs(aux-aux2);
}
...
int main_gauss_seidel(){
    ...
    cout<<"Dimension de la matriz A: ";
    cin>>dim;
    float A[dim][dim],B[dim][dim],b[dim],x[dim],x_prev[dim],aux[dim],errrr;

    cout<<endl<<" Elementos de la matriz A: "<<endl;
    for(i=0;i<dim;i++) for(j=0;j<dim;j++){
        cout<<"A("<<i<<","<<j<<")"; cin>>A[i][j];B[i][j]=A[i][j]; cout<<endl;
    }
    ...
    cout<<"k"<<"|"<<"x[i]"<<"|"<<"|"<<"y[i]"<<"|"<<"|"<<"z[i]"<<"|"<<"||Xk-Xk-1||"<<"|"<<"||Yk-Yk-1||"<<"|"<<"||Zk-Zk-1||"<<endl;
    //for(i=0;i<dim;i++)
       // cout<<"x "<<i+1<<endl;
    do{
        cout<<count+1;
